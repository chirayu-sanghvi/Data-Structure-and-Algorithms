...
Leet Code: https://leetcode.com/problems/find-the-smallest-divisor-given-a-threshold/description/
...
// Linear Search Solution (Partially Accepted)
public class Solution {
    public static int smallestDivisor(int arr[], int limit) {
        ...
        while(mini <= maxi){
            int sum = 0;
            for(int i =0;i<n;i++){
                sum += Math.ceil((double)arr[i]/mini);
            }
            if(sum <= limit){
                return mini;
            }
...
// Binary Search Solution (Completely Accepted)
public class Solution {
    public static int smallestDivisor(int arr[], int limit) {
        // Write your coder here
        int n = arr.length;
        ...
        while(mini <= maxi){
            mid = (int)(mini+maxi)/2;
            boolean find = findlimit(arr,mid,limit);
            if(find == true){
                answer = mid;
                maxi = mid-1;
            }
    ...
    public static boolean findlimit(int[] arr, int mini, int limit){
            int sum = 0;
